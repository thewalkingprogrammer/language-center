...
if (@$_GET['lang'] == 1) {
    ...
    . '<h3 class="panel-title">Good Luck!</h3>'
    . '</div>'
    . '<div class="panel-body"><h1>Japanese Test</h1>';
} else if (@$_GET['lang'] == 2) {
    ...
    . '<h3 class="panel-title">Good Luck!</h3>'
    . '</div>'
    . '<div class="panel-body"><h1>Korean Test</h1>';
}

if ($_SERVER['REQUEST_METHOD'] == "GET") {
    $_SESSION['tempLangIdTest'] = $_GET['lang'];
    $query = "select questionId, a1, a2, a3, ca, question "
            . "from questions "
            . "where langId = " . $_SESSION['tempLangIdTest'];
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
            echo '<h3>Question # ' . $questionNumber . '</h3><br /><h5>' . $row['question'] . '</h5>';
...
if (isset($_POST['submit_test'])) {
    include ('includes/header.php');
    $query = "select questionId, ca, question "
            . "from questions "
            . "where langId = " . $_POST['lang'];
    $result = mysqli_query($con, $query);
    $score = 0;
    $questionNumber = 1;
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $query = "insert into scores (score, userId) "
                . "values ( {$score}, {$_POST['user']})";
        $result = mysqli_query($con, $query);
